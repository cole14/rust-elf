...
/// a `AsyncRead + AsyncSeek`.
#[derive(Debug)]
pub struct ElfAsyncStream<E: EndianParse, S: AsyncRead + AsyncSeek + Unpin> {
...
async fn parse_section_headers<E: EndianParse, S: AsyncRead + AsyncSeek + Unpin>(
    ehdr: &FileHeader<E>,
    ...
    if shnum == 0 {
        ...
        let mut offset = 0;
        let data = reader.read_bytes(shoff, end).await?;
    ...
    let buf = reader.read_bytes(shoff, end).await?;
...
async fn parse_program_headers<E: EndianParse, S: AsyncRead + AsyncSeek + Unpin>(
    ehdr: &FileHeader<E>,
    reader: &mut CachingReader<S>,
    ...
    let buf = reader.read_bytes(phoff, end).await?;
...
impl<E: EndianParse, S: AsyncRead + AsyncSeek + Unpin> ElfAsyncStream<E, S> {
    /// Do a minimal amount of parsing work to open an [ElfAsyncStream] handle from a Read+Seek containing an ELF object.
    ///
    ...
    pub async fn open_stream(reader: S) -> Result<ElfAsyncStream<E, S>, ParseError> {
        let mut cr = CachingReader::new(reader).await?;
        let ident_buf = cr.read_bytes(0, abi::EI_NIDENT).await?;
        ...
        let tail_buf = cr.read_bytes(tail_start, tail_end).await?;

        let ehdr = FileHeader::parse_tail(ident, tail_buf)?;

        let shdrs = parse_section_headers(&ehdr, &mut cr).await?;
        let phdrs = parse_program_headers(&ehdr, &mut cr).await?;
    ...
    pub async fn segments(&self) -> &Vec<ProgramHeader> {
        &self.phdrs
    }

    /// Get the parsed section headers table
    pub async fn section_headers(&self) -> &Vec<SectionHeader> {
        &self.shdrs
    }
    ...
    pub async fn section_headers_with_strtab(
        &mut self,
        ...
        let strtab_buf = self.reader.read_bytes(strtab_start, strtab_end).await?;
    ...
    ///   let io = async_fs::File::open(path).await.expect("Could not open file.");
    ///   let mut file = ElfAsyncStream::<AnyEndian, _>::open_stream(io).await.expect("Open test1");
    ///
    ...
    /// # });
    /// ```
    pub async fn section_header_by_name(
        &mut self,
        name: &str,
    ) -> Result<Option<&SectionHeader>, ParseError> {
        let (shdrs, strtab) = match self.section_headers_with_strtab().await? {
    ...
    pub async fn section_data(
        ...
        let buf = self.reader.read_bytes(start, end).await?;
    ...
    pub async fn section_data_as_strtab(
        &mut self,
        shdr: &SectionHeader,
    ...
    async fn get_symbol_table_of_type(
        &mut self,
        symtab_type: u32,
        ...
        match self.shdrs.iter().find(|shdr| shdr.sh_type == symtab_type) {
            Some(shdr) => {
                ...
                self.reader.load_bytes(symtab_start..symtab_end).await?;
                ...
                let (strtab_start, strtab_end) = strtab.get_data_range()?;
                self.reader.load_bytes(strtab_start..strtab_end).await?;
    ...
    pub async fn symbol_table(
        &mut self,
    ) -> Result<Option<(SymbolTable<'_, E>, StringTable<'_>)>, ParseError> {
        self.get_symbol_table_of_type(abi::SHT_SYMTAB).await
    }
    ...
    pub async fn dynamic_symbol_table(
        &mut self,
    ) -> Result<Option<(SymbolTable<'_, E>, StringTable<'_>)>, ParseError> {
        self.get_symbol_table_of_type(abi::SHT_DYNSYM).await
    }

    /// Get the .dynamic section/segment contents.
    pub async fn dynamic(&mut self) -> Result<Option<DynamicTable<'_, E>>, ParseError> {
    ...
    pub async fn symbol_version_table(
        &mut self,
        ...
        let needs_shdrs = match needs_opt {
            Some(shdr) => {
                ...
                self.reader.load_bytes(strs_start..strs_end).await?;
    ...
    /// [sh_type](SectionHeader#structfield.sh_type) is not
    /// [SHT_REL](abi::SHT_REL).
    pub async fn section_data_as_rels(
        &mut self,
        shdr: &SectionHeader,
    ...
    pub async fn section_data_as_relas(
        &mut self,
        shdr: &SectionHeader,
    ...
    pub async fn section_data_as_notes(
        &mut self,
        shdr: &SectionHeader,
    ...
    pub async fn segment_data(&mut self, phdr: &ProgramHeader) -> Result<&[u8], ParseError> {
        let (start, end) = phdr.get_file_data_range()?;
        self.reader.read_bytes(start, end).await
    }
    ...
    /// [PT_NOTE](abi::PT_NOTE).
    pub async fn segment_data_as_notes(
        &mut self,
        phdr: &ProgramHeader,
    ) -> Result<NoteIterator<'_, E>, ParseError> {
    ...
    /// Releases the reader from the [ElfAsyncStream] struct
    pub async fn release(self) -> S {
        self.reader.reader
    }
}

#[derive(Debug)]
struct CachingReader<R: AsyncRead + AsyncSeek> {
    reader: R,
    stream_len: u64,
    bufs: HashMap<(usize, usize), Box<[u8]>>,
}

impl<R: AsyncRead + AsyncSeek + Unpin> CachingReader<R> {
    async fn new(mut reader: R) -> Result<Self, ParseError> {
    ...
    async fn read_bytes(&mut self, start: usize, end: usize) -> Result<&[u8], ParseError> {
        self.load_bytes(start..end).await?;
    ...
    async fn load_bytes(&mut self, range: Range<usize>) -> Result<(), ParseError> {
        ...
        self.reader.seek(SeekFrom::Start(range.start as u64)).await?;
...
mod interface_tests {
    ...
    async fn test_open_stream() {
        ...
        let path = std::path::PathBuf::from("sample-objects/basic.x86_64");
        let io = async_fs::File::open(path).await.expect("Could not open file.");
        let file = ElfAsyncStream::<AnyEndian, _>::open_stream(io).await.expect("Open test1");
        assert_eq!(file.ehdr.e_type, abi::ET_EXEC);
    }

    #[futures_test::test]
    async fn section_headers_with_strtab() {
        let path = std::path::PathBuf::from("sample-objects/basic.x86_64");
        let io = async_fs::File::open(path).await.expect("Could not open file.");
        let mut file = ElfAsyncStream::<AnyEndian, _>::open_stream(io).await.expect("Open test1");
    ...
    #[futures_test::test]
    async fn shnum_and_shstrndx_in_shdr0() {
    ...
    #[futures_test::test]
    async fn section_header_by_name() {
        let path = std::path::PathBuf::from("sample-objects/basic.x86_64");
        let io = async_fs::File::open(path).await.expect("Could not open file.");
        let mut file = ElfAsyncStream::<AnyEndian, _>::open_stream(io).await.expect("Open test1");
    ...
    #[futures_test::test]
    async fn section_data_for_nobits() {
        let path = std::path::PathBuf::from("sample-objects/basic.x86_64");
        let io = async_fs::File::open(path).await.expect("Could not open file.");
        let mut file = ElfAsyncStream::<AnyEndian, _>::open_stream(io).await.expect("Open test1");

        let shdr = file.section_headers().await[26];
    ...
    async fn section_data() {
        ...
        let shdr = file.section_headers().await[7];
    ...
    #[futures_test::test]
    async fn section_data_as_strtab() {
        let path = std::path::PathBuf::from("sample-objects/basic.x86_64");
        let io = async_fs::File::open(path).await.expect("Could not open file.");
        let mut file = ElfAsyncStream::<AnyEndian, _>::open_stream(io).await.expect("Open test1");

        let shdr = file.section_headers().await[file.ehdr.e_shstrndx as usize];
        let strtab = file
    ...
    async fn segments() {
        ...
        let segments = file.segments().await;
        assert_eq!(
            segments[0],
            ProgramHeader {
                p_type: abi::PT_PHDR,
    ...
    #[futures_test::test]
    async fn section_data_as_rels() {
        let path = std::path::PathBuf::from("sample-objects/basic.x86_64");
        let io = async_fs::File::open(path).await.expect("Could not open file.");
        let mut file = ElfAsyncStream::<AnyEndian, _>::open_stream(io).await.expect("Open test1");

        let shdr = file.section_headers().await[10];
    ...
    #[futures_test::test]
    async fn section_data_as_relas() {
        let path = std::path::PathBuf::from("sample-objects/basic.x86_64");
        let io = async_fs::File::open(path).await.expect("Could not open file.");
        let mut file = ElfAsyncStream::<AnyEndian, _>::open_stream(io).await.expect("Open test1");
    ...
    #[futures_test::test]
    async fn section_data_as_notes() {
        let path = std::path::PathBuf::from("sample-objects/basic.x86_64");
        let io = async_fs::File::open(path).await.expect("Could not open file.");
        let mut file = ElfAsyncStream::<AnyEndian, _>::open_stream(io).await.expect("Open test1");

        let shdr = file.section_headers().await[2];
        let mut notes = file
            .section_data_as_notes(&shdr)
            .await
            .expect("Failed to read relas section");
    ...
    #[futures_test::test]
    async fn segment_data_as_notes() {
        let path = std::path::PathBuf::from("sample-objects/basic.x86_64");
        let io = async_fs::File::open(path).await.expect("Could not open file.");
        let mut file = ElfAsyncStream::<AnyEndian, _>::open_stream(io).await.expect("Open test1");
    ...
    async fn release() {
        ...
        let mut released_io = file.release().await;
        released_io.seek(SeekFrom::Start(0)).await.expect("should seek");
        let mut bytes = vec![0; abi::EI_NIDENT].into_boxed_slice();
        released_io.read_exact(&mut bytes).await.expect("should read");
...
mod arch_tests {
    ...
    macro_rules! arch_test {
        ( $arch:expr, $e_machine:expr, $endian:expr) => {async {
            let path_str = format!("sample-objects/symver.{}.so", $arch);
            let path = std::path::PathBuf::from(path_str);
            let io = async_fs::File::open(path).await.expect("file should exist");
            let mut file = ElfAsyncStream::<AnyEndian, _>::open_stream(io).await.expect("should parse");

            assert_eq!(file.ehdr.e_machine, $e_machine);
            assert_eq!(file.ehdr.endianness, $endian);

            let (shdrs, strtab) = file.section_headers_with_strtab().await.expect("should parse");
            let (shdrs, strtab) = (shdrs, strtab.unwrap());
            let _: Vec<_> = shdrs
                .iter()
                .map(|shdr| {
            ...
            if let Some((symtab, strtab)) = file.symbol_table().await.expect("should parse") {
                let _: Vec<_> = symtab
                    .iter()
            ...
            if let Some((symtab, strtab)) = file.dynamic_symbol_table().await.expect("should parse") {
                let _: Vec<_> = symtab
                    .iter()
                    .map(|sym| (strtab.get(sym.st_name as usize).expect("should parse"), sym))
                    .collect();
    ...
    #[futures_test::test]
    async fn x86_64() {
        arch_test!("x86_64", abi::EM_X86_64, AnyEndian::Little).await;
    }

    #[futures_test::test]
    async fn m68k() {
        arch_test!("m68k", abi::EM_68K, AnyEndian::Big).await;
    }

    #[futures_test::test]
    async fn aarch64() {
        arch_test!("aarch64", abi::EM_AARCH64, AnyEndian::Little).await;
    }

    #[futures_test::test]
    async fn armhf() {
        arch_test!("armhf", abi::EM_ARM, AnyEndian::Little).await;
    }

    #[futures_test::test]
    async fn powerpc64() {
        arch_test!("powerpc64", abi::EM_PPC64, AnyEndian::Big).await;
    }

    #[futures_test::test]
    async fn powerpc64le() {
        arch_test!("powerpc64le", abi::EM_PPC64, AnyEndian::Little).await;
    }

    #[futures_test::test]
    async fn riscv64() {
        arch_test!("riscv64", abi::EM_RISCV, AnyEndian::Little).await;
    }
}
